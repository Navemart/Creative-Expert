...
function StepNav({ step, total, onBack, onNext, nextLabel, nextColor = '#F5C118', disabled = false }) {
  return (
    <div className="flex items-center justify-between mt-6 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      <span className="text-base" style={{ color: 'rgba(255,255,255,0.3)' }}>שלב {step} מתוך {total}</span>
      <button
        onClick={onNext}
        disabled={disabled}
      ...
      {Array.from({ length: 11 }, (_, i) => {
        ...
        return (
          <button
            key={i}
            onClick={() => onChange(i)}
            className="h-8 w-8 rounded-lg text-base font-semibold transition-all"
            style={{
  ...
  const steps = [
        ...
        {QUARTER_OPTIONS.map(q => {
          ...
          return (
            <button
              ...
              className="rounded-xl px-3 py-2.5 text-base font-semibold text-right transition-all relative"
              style={{
          ...
          <div>
            ...
            <div className="flex items-center gap-1 rounded-xl px-3 py-2.5" style={{ background: 'rgb(var(--bg-elevated))', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <input type="number" value={form.highest_cash_month} onChange={e => set('highest_cash_month', e.target.value)}
                className="flex-1 bg-transparent text-base text-white outline-none" placeholder="10000" />
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-1 rounded-xl px-3 py-2.5" style={{ background: 'rgb(var(--bg-elevated))', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <input type="number" value={form.goal_cash_month} onChange={e => set('goal_cash_month', e.target.value)}
                className="flex-1 bg-transparent text-base text-white outline-none" placeholder="20000" />
            </div>
          </div>
        </div>
        <div>
        ...
        {EFFORT_METRICS.map(m => (
          <div key={m.key} className="rounded-xl p-4 space-y-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center justify-between">
              ...
              <span className="text-base font-bold" style={{ color: m.color }}>{form[m.key]}/10</span>
            </div>
      ...
      <div className="space-y-3">
        {['fun_month_1','fun_month_2','fun_month_3'].map((k, i) => (
          <div key={k} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <p className="text-base font-bold text-white mb-2">{months[i]}</p>
            <input
              type="text"
              value={form[k]}
    ...
    <div key="review">
      <div className="mb-5">
        ...
        <p className="text-base mt-1" style={{ color: 'rgba(255,255,255,0.45)' }}>בדוק הכל — ברגע שנשלח זה נעול.</p>
      </div>
      <div className="space-y-4 text-base overflow-y-auto" style={{ maxHeight: '50vh' }}>
        {/* Quarter */}
      ...
      {err && <p className="text-base mt-3" style={{ color: '#ef4444' }}>{err}</p>}

      <div className="flex items-center justify-between mt-6 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        <button onClick={() => setStep(4)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-base font-medium hover:bg-white/10" style={{ color: 'rgba(255,255,255,0.55)' }}>
        ...
        <button
          onClick={submit}
          disabled={saving}
          className="flex items-center gap-2 px-5 py-2 rounded-xl text-base font-bold"
...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-xl p-3 text-base space-y-1.5" style={{ background: 'rgb(var(--bg-elevated))', border: '1px solid rgba(255,255,255,0.1)', direction: 'rtl' }}>
...
function SubmissionDetail({ record, onClose }) {
  ...
  const Row = ({ label, value, color }) => value ? (
    <div className="flex gap-4 py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      <span className="w-44 flex-none text-base" style={{ color: 'rgba(255,255,255,0.4)' }}>{label}</span>
      <span className="flex-1 text-base leading-relaxed" style={{ color: color || 'rgba(255,255,255,0.85)' }}>{value}</span>
    </div>
  ) : null;
    ...
    <div className="p-8 space-y-8" dir="rtl">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-base font-bold flex-none btn-yellow"
      ...
      <div className="rounded-2xl p-6 space-y-5" style={{ background: 'rgb(var(--bg-surface))', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div>
          ...
          <p className="text-base mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>ציון עצמי 0–10 כל רבעון.</p>
        </div>
        ...
        {isEmpty ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <button
              onClick={() => setShowModal(true)}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-base font-bold btn-yellow"
              style={{ background: 'rgb(var(--accent))', color: 'rgb(var(--accent-foreground))' }}
            >
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData} margin={{ top: 10, right: 16, bottom: 0, left: -16 }}>
              ...
              <Legend wrapperStyle={{ paddingTop: 20, direction: 'rtl', fontSize: '0.875rem' }} formatter={v => <span style={{ color: 'rgba(255,255,255,0.6)' }}>{v}</span>} />
              {EFFORT_METRICS.map(m => (
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-base font-bold text-white">{quarterLabel(r.quarter)}</span>
                </div>
                ...
                <div className="space-y-1.5 flex-1">
                  {r.goal_cash_month && (
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="text-base font-semibold text-white">₪{Number(r.goal_cash_month).toLocaleString()}</span>
                    </div>
                  )}
                  {r.clients_to_sign && (
                    <div className="flex items-baseline justify-between gap-2">
                      <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>לקוחות לסגירה</span>
                      <span className="text-base font-semibold text-white">{r.clients_to_sign}</span>
                    </div>
                  )}
                  {lvl && (
                    <div className="flex items-baseline justify-between gap-2">
                      <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>דרגה נוכחית</span>
                      <span className="text-base font-semibold" style={{ color: lvl.color }}>{lvl.label}</span>
                    </div>
                  )}
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>מאמץ — קצר / שיחות / הצעות</span>
                    <span className="text-base font-semibold text-white">{r.effort_short} / {r.effort_relations} / {r.effort_proposals}</span>
                  </div>
                </div>
