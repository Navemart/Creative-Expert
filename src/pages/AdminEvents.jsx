        ...
        <div className="flex items-center gap-2">
          {['table', 'stats'].map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{ padding: '6px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: '1px solid rgba(255,255,255,0.12)', background: tab === t ? 'rgba(245,193,24,0.15)' : 'transparent', color: tab === t ? '#F5C118' : 'rgba(255,255,255,0.5)' }}>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {[
          ...
          <input key={key} type={type} placeholder={placeholder} value={filters[key]}
            ...
            style={{ width, padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'rgb(var(--bg-surface))', color: 'white', fontSize: 12, outline: 'none' }}
          />
        ))}
        <button onClick={load} style={{ padding: '6px 14px', borderRadius: 8, background: '#F5C118', color: '#13152A', fontWeight: 700, fontSize: 12, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          <Filter size={13} /> סנן
        </button>
        <button onClick={() => { setFilters({ user: '', event: '', page: '', from: '', to: '' }); }}
          style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'transparent', color: 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
          <X size={12} /> נקה
        </button>
      </div>
                ...
                {loading ? (
                  ...
                  <tr key={ev.id} style={{ background: ev.ignore_on ? 'rgba(255,255,255,0.02)' : 'transparent', opacity: ev.ignore_on ? 0.4 : 1 }}>
                    ...
                    <td style={{ ...cell, maxWidth: 180 }}>
                      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace' }} title={ev.page}>
                        {PAGE_LABELS[ev.page] || ev.page || '—'}
                      </span>
                    </td>
                    <td style={{ ...cell, maxWidth: 220 }}>
                      {ev.metadata ? (
                        <span style={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(255,255,255,0.4)' }} title={JSON.stringify(ev.metadata)}>
                    ...
                    <td style={cell}>
                      <span style={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(255,255,255,0.3)' }} title={ev.session_id}>
                    ...
                    <td style={cell}>
                      <button onClick={() => toggleIgnore(ev.id, ev.ignore_on)}
                        style={{ fontSize: 10, padding: '2px 8px', borderRadius: 5, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(255,255,255,0.3)', cursor: 'pointer' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'white' }}>לפי סוג אירוע</h3>
            </div>
            <div style={{ padding: '8px 0' }}>
              {stats.byEvent.map(([ev, count]) => (
                <div key={ev} className="flex items-center justify-between" style={{ padding: '8px 16px' }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'white' }}>{count.toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'white' }}>עמודים פופולריים</h3>
            </div>
            <div style={{ padding: '8px 0' }}>
                ...
                return (
                  <div key={pg} style={{ padding: '6px 16px' }}>
                    <div className="flex items-center justify-between mb-1">
                      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)' }}>{PAGE_LABELS[pg] || pg}</span>
          ...
          {stats.daily.length > 0 && (
            <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', gridColumn: '1 / -1' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: 'white' }}>צפיות יומיות — 30 ימים אחרונים</h3>
              </div>
